...
private void openAddForm() {
    ...
    String maSinhVien = generateStudentId();
    ...
    Object[] message = {
            "Mã Sinh Viên (Tự động):", maSinhVien,
...
// Hàm tự động tạo mã sinh viên
private String generateStudentId() {
    String currentYear = String.valueOf(java.util.Calendar.getInstance().get(java.util.Calendar.YEAR)).substring(2);
    String prefix = currentYear + "TIN";

    try {
        String sql = "SELECT id FROM sinh_vien WHERE id LIKE ? ORDER BY id DESC LIMIT 1";
        PreparedStatement ps = conn.prepareStatement(sql);
    ...
    // Nếu không có mã sinh viên nào trước đó
    return prefix + "001";
}
